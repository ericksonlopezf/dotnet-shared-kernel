...
flowchart TB
    subgraph Tier1["Tier 1: Fast Inner Loop (<2.5s Feedback)"]
        ...
        SourceGen["Source Generator Tests (Roslyn AST)<br/>• DapperRegistrationGenerator & MetadataSourceGenerator<br/>• Semantic Invariants & Diagnostics"]
...
| `EricksonLopez.SharedKernel.SourceGenerators.Tests` | Source Generator (Roslyn) | Tests covering incremental source generation for `DapperRegistrationGenerator` and `MetadataSourceGenerator`, attribute discovery, diagnostic reporting, syntax tree generation, and compiler AST mutation resilience. (StrongId generation centralized in `EricksonLopez.DomainPrimitives.Generators`). | 14 | ~0.5s |
