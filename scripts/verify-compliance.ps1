...
<#
.SYNOPSIS
    Automated zero-tolerance repository compliance & governance verifier for EricksonLopez.SharedKernel.
.DESCRIPTION
    Validates:
...
foreach ($f in $allTrackedFiles) {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        if ($lines[$i] -match "github\.com/ericksonlopez/dotnet-shared-kernel\b") {
...
if ($badLinks -eq 0) { Write-Host "  ✅ All GitHub URLs correctly target ericksonlopezf/dotnet-shared-kernel." -ForegroundColor Green }
...
$illegalNoWarn = 0
$projFiles = Get-ChildItem -Path $RootDirectory -Recurse -Include "*.csproj", "*.props" | Where-Object { $_.FullName -notmatch '[\\/](bin|obj|StrykerOutput|BenchmarkDotNet\.Artifacts|node_modules)[\\/]' }
